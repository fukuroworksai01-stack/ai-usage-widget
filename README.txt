...
■ 初期設定（取得元の資格情報）
  ...
  Claude 側（sessionKey の入れ方は3通り）:
    ...
    (2) 「ログインして取得」… Chrome/Edge 推奨。専用ウィンドウが開くので
        ...
        （Chrome 127+ の暗号化にも、ブラウザ自身のセッションを使って対応）。
...
■ 操作
  - フォントは自動でモダンなUIフォントを採用します（Inter / Segoe UI Variable 等が
    あればそれを、無ければ Segoe UI）。より今時の見た目にしたい場合は Inter を
    インストールしておくと自動で使われます。
  - 各枠は円形リングゲージで残量%（緑→黄→赤）を表示。複数枠は横並びでコンパクト。
    クレジット制は「残/総（リング）」または総量不明なら残クレジット数を表示します。
    リセットまでの残り時間は毎分ライブで減っていきます。
  - 右クリックメニュー: 表示倍率 / 位置をロック / 設定 / 自動実行 / バージョン情報 / 終了
  - 設定の「透明度」スライダーでウィンドウの不透明度を調整できます（40〜100%）。
  ...
  - 右上 ⚙ で設定ダイアログ、⟳ で更新、－ で最小化（タスクバーに収納）、✕ で終了
    ※ 最小化するとタスクバーにボタンが出ます（クリックで元に戻ります）
  - リングにマウスを乗せると、正確な数値（残り%/クレジット残数・リセット時刻）を
    ツールチップ表示します。
  - 取得に失敗しても直前に取れた値を「⚠ 更新失敗・HH:MM時点の値」として
    表示し続けます（毎回エラーで点滅しません）。一時的な失敗（通信/Cloudflare等）
    のときは通常間隔を待たず、数十秒間隔で自動リトライして復帰します。
